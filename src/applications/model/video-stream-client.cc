...
VideoStreamClient::StartApplication (void)
{
  ...
  std:: cout<< "SocketConnection established"<<std::endl;
...
{
   std::cout<<"At time " << Simulator::Now ().GetSeconds () << " s, last buffer size: " << m_lastBufferSize << ", current buffer size: " << m_currentBufferSize<< std::endl;
...
VideoStreamClient::HandleRead (Ptr<Socket> socket)
{
  ...
  while ((packet = socket->RecvFrom (from)))
  {
    ...
    if (InetSocketAddress::IsMatchingType (from))
    {
      ...
      std::cout<<"m_framesize inside handleRead "<< m_frameSize <<std::endl; 
